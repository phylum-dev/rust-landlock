...
///     ruleset = ruleset.add_rules(path_beneath_rules(&["/usr", "/etc", "/dev"], AccessFs::from_read(abi)))?;
///     // Read-write access to /home and /tmp.
///     ruleset = ruleset.add_rules(path_beneath_rules(&["/home", "/tmp"], AccessFs::from_all(abi)))?;
